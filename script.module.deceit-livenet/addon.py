...
def get_auth_token_33(referer):
    ...
    r = requests.get(wms_url, headers={"Accept": "", "User-Agent": user_agent, "Referer": referer, "Modified": modified(mod_value), "Authorization": auth})
...
def get_auth_token_44():
    ...
    r = requests.get(wms_url, headers={"Accept": "", "User-Agent": user_agent, "Modified": modified(mod_value), "Authorization": auth})
...
def get_stream_32(stream):
    ...
    r = requests.get(response_body_api_url, headers={"Accept": "", "User-Agent": user_agent, "Authorization": response_body_auth})
    response_body = r.text

    data = {"data": json.dumps({"token": 32, "response_body": response_body, "stream_url": stream})}

    r = requests.post(api_url, headers={"Accept": "", "User-Agent": user_agent, "Modified": modified(mod_value), "Authorization": auth}, data=data)
...
def play(c_id):
    ...
    if selected_stream.get("player_user_agent", user_agent) == None or selected_stream.get("player_user_agent", user_agent) == "null":
